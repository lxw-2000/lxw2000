...
<#if articleVoIPage?? && articleVoIPage.list?size gt 0 >
    <form class="form-horizontal" action="/lxw2000/article/list" method="get">
        <div class="form-group">
            ...
            <div class="cpl-sm-2">
                ...
                <a href="/lxw2000/article/list" class="btn btn-success"><i class="icon-search"></i> 全部</a>
            </div>
        </div>
    </form>
    <div class="panel">
...
<script type="text/javascript">
    function hotArticle(articleId) {
        if (confirm("是否设置（取消）热门文章")) {
            if (!checkNotNull(articleId)) {
                zuiMsg('程序出错，请刷新页面重试');
                return;
            }
            $.post("/lxw2000/article/hot", {
                    articleId: articleId
                },
                function (data) {
                    if (data.code == 200) {
    ...
    function getNewData(pageNumber) {
        ...
        window.location.href = "/lxw2000/article/list?pageNumber=" + pageNumber + "<#if (articleName?? && articleName?length>0)>&articleName=${articleName!}</#if>";
    }
